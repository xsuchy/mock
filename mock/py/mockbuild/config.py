...
def set_config_opts_per_cmdline(config_opts, options, args):
    ...
    process_hermetic_build_config(options, config_opts)
...
def update_config_from_dict(config_opts, updates):
    """
    Merge a dictionary into config_opts.  No include supported.
    """
    for key, value in updates.items():
        config_opts[key] = value


def process_hermetic_build_config(cmdline_opts, config_opts):
    """
    Read the lockfile file generated by the previous
    --calculate-build-dependencies run, and adjust the current set of options
    in CONFIG_OPTS.
    """

    if not cmdline_opts.hermetic_build:
        return

    config_opts["hermetic_build"] = True

    json_conf, repo_reference = cmdline_opts.hermetic_build
    with open(json_conf, "r", encoding="utf-8") as fd:
        data = json.load(fd)

    if not data["config"].get("bootstrap_image_ready"):
        raise exception.BadCmdline(
            f"The file {json_conf} did not record the bootstrap_image_ready=True "
            "config which means we are not able to prepare the bootstrap chroot "
            "in a hermetic mode.")

    update_config_from_dict(config_opts, data["config"])

    final_offline_repo = repo_reference
    file_pfx = "file://"
    if final_offline_repo.startswith(file_pfx):
        final_offline_repo = final_offline_repo[len(file_pfx):]
    final_offline_repo = os.path.abspath(final_offline_repo)
    if not os.path.exists(os.path.join(final_offline_repo, "repodata")):
        raise exception.BadCmdline(
